/* global __react_refresh_error_overlay_alt__, __react_refresh_socket_alt__, __resourceQuery */
...
function tryDismissErrorOverlay() {
  __react_refresh_error_overlay_alt__.clearCompileError();
  __react_refresh_error_overlay_alt__.clearRuntimeErrors(!hasRuntimeErrors);
...
function handleCompileErrors(errors) {
  ...
  __react_refresh_error_overlay_alt__.showCompileError(formattedErrors[0]);
}
...
if (process.env.NODE_ENV !== 'production') {
  if (typeof window !== 'undefined') {
    runWithPatchedUrl(function setupOverlay() {
      ...
      if (!window.__reactRefreshOverlayInjected && __react_refresh_socket_alt__) {
        ...
        runWithRetry(function initSocket() {
          __react_refresh_socket_alt__.init(compileMessageHandler, __resourceQuery);
        }, 3);
        // Registers handlers for runtime errors
        events.handleError(function handleError(error) {
          hasRuntimeErrors = true;
          __react_refresh_error_overlay_alt__.handleRuntimeError(error);
        });
